...
Машинный код для link:examples/hello.asm[]:
...
Запись из АЛУ в регистры также реализована ввиду метода `write_register`, который вызывается каждый такт. Аргументами метода являются `*_sel` и `alu_ctrl`
...
* `alu-ctrl` - Выбор операции выполняемой АЛУ. По умолчанию `only_x`, то есть просто возврат первого операнда
...
Пример использования и журнал работы процессора на примере link:examples/cat.asm:
...
src/golden_test.py::test_translator_asm_and_machine[golden/fib.yml] PASSED                                                              [ 25%]
src/golden_test.py::test_translator_asm_and_machine[golden/hello.yml] PASSED                                                            [ 50%]
src/golden_test.py::test_translator_asm_and_machine[golden/hello_user_name.yml] PASSED                                                  [ 75%]
src/golden_test.py::test_translator_asm_and_machine[golden/cat.yml] PASSED                                                              [100%]
